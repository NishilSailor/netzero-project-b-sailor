...
app.get('/quote',        (req, res) => res.render('quote', { error: false }));
...
app.get('/admin/dashboard', requireAdmin, async (req, res) => {
  try {
    const quotes  = await pool.query(`SELECT * FROM quote_requests ORDER BY submitted_at DESC`);
...
app.post('/admin/quote/status', requireAdmin, async (req, res) => {
  ...
  try {
    await pool.query(`UPDATE quote_requests SET status = $1 WHERE id = $2`, [status, id]);
    res.redirect('/admin/dashboard');
...
app.get('/admin/policies', requireAdmin, async (req, res) => {
  try {
    const policies = await pool.query(
      `SELECT policies.*, clients.full_name AS client_name
       FROM policies JOIN clients ON policies.client_id = clients.id
       ORDER BY policies.id DESC`
    );
    const clients = await pool.query(`SELECT id, full_name FROM clients ORDER BY full_name`);
    res.render('admin/policies', {
      admin: req.session.admin,
      policies: policies.rows,
      clients: clients.rows
    });
  } catch (err) { console.error(err); res.send('Error loading policies'); }
});

// ── Admin: Add Policy ─────────────────────────────────────────
app.post('/admin/policy/add', requireAdmin, async (req, res) => {
  const { client_id, policy_type, insurer, premium, start_date, renewal_date } = req.body;
  try {
    await pool.query(
      `INSERT INTO policies (client_id, policy_type, insurer, premium, start_date, renewal_date)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [client_id, policy_type, insurer, premium, start_date, renewal_date]
    );
    res.redirect('/admin/policies');
  } catch (err) { console.error(err); res.redirect('/admin/policies'); }
});

// ── Admin: Delete Policy ──────────────────────────────────────
app.post('/admin/policy/delete', requireAdmin, async (req, res) => {
  const { id } = req.body;
  try {
...
// ── Admin: Brokers Page ───────────────────────────────────────
app.get('/admin/brokers', requireAdmin, async (req, res) => {
  try {
    const brokers = await pool.query(
      `SELECT brokers.*,
        COUNT(clients.id) AS client_count
       FROM brokers
       LEFT JOIN clients ON clients.assigned_broker_id = brokers.id
       GROUP BY brokers.id
       ORDER BY brokers.name`
    );
    res.render('admin/brokers', {
      admin: req.session.admin,
      brokers: brokers.rows
    });
  } catch (err) { console.error(err); res.send('Error loading brokers'); }
});

// ── Admin: Add Broker ─────────────────────────────────────────
app.post('/admin/broker/add', requireAdmin, async (req, res) => {
  const { name, email, specialization } = req.body;
  try {
    await pool.query(
      `INSERT INTO brokers (name, email, specialization) VALUES ($1, $2, $3)`,
      [name, email, specialization]
    );
    res.redirect('/admin/brokers');
  } catch (err) { console.error(err); res.redirect('/admin/brokers'); }
});

// ── Admin: Delete Broker ──────────────────────────────────────
app.post('/admin/broker/delete', requireAdmin, async (req, res) => {
  const { id } = req.body;
  try {
...
// ── Client Auth ───────────────────────────────────────────────
app.get('/client/login', (req, res) => res.render('client/login', { error: false }));
...
app.get('/client/dashboard', requireClient, async (req, res) => {
  try {
    ...
    const brokerResult = await pool.query(
      `SELECT brokers.* FROM brokers
       JOIN clients ON clients.assigned_broker_id = brokers.id
       WHERE clients.id = $1`,
      [clientId]
    );
    ...
    res.render('client/dashboard', {
      ...
      broker:   brokerResult.rows[0] || null
    });
  } catch (err) {
    console.error(err);
    res.send('Error loading dashboard');
...
app.post('/client/message/send', requireClient, async (req, res) => {
  ...
  try {
    ...
    res.redirect('/client/dashboard');
  } catch (err) { console.error(err); res.redirect('/client/dashboard'); }
});
